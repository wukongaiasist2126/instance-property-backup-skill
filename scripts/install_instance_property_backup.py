...
def mirror_user_entry(output_html: Path, user_entry_dir: Path, instance_name: str) -> Path:
    instance_entry_dir = user_entry_dir / instance_name
    instance_entry_dir.mkdir(parents=True, exist_ok=True)
    ...
    user_html = instance_entry_dir / final_name
    shutil.copy2(output_html, user_html)
    for legacy_name in legacy_names:
        legacy_path = instance_entry_dir / legacy_name
...
def remove_old_launchers(instance_entry_dir: Path, keep_names: set[str], instance_label: str) -> None:
    instance_prefix = instance_label.lower()
    for path in instance_entry_dir.iterdir():
        if not path.is_file():
            continue
        if path.name in keep_names:
            continue
        lower = path.name.lower()
        if not lower.startswith(instance_prefix):
            continue
...
def main() -> int:
    ...
    instance_entry_dir = user_entry_dir / args.instance_name
    instance_entry_dir.mkdir(parents=True, exist_ok=True)
    instance_label = build_instance_label(args.instance_name, workspace)
    backup_root = Path(args.backup_root)
    instance_dir = backup_root / args.instance_name
    ...
    values = {
        ...
        'user_entry_dir': str(instance_entry_dir),
    ...
    cmd_launcher = instance_entry_dir / f'{instance_label}_属性备份.cmd'
    ...
    command_launcher = instance_entry_dir / f'{instance_label}_属性备份.command'
    ...
    remove_old_launchers(instance_entry_dir, {cmd_launcher.name, command_launcher.name}, instance_label)
